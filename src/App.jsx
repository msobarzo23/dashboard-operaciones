...
// ═══ VIEW 9: COMBUSTIBLE (versión reescrita) ═══
// Reemplaza la función Combustible existente en App.jsx con este bloque completo.
// Dependencias ya presentes en el archivo:
//   - useState, useEffect, useMemo, useCallback (React)
//   - StatCard, SucBadge, SortTh, Pager, useSortable
//   - isVacioTrip, getSucursal, dayKey, getMonthKey, monthKeyToLabel, formatDate
...
function Combustible({data, flota, today, T}) {
  ...
  // ─── Períodos actual vs previo equivalente ───
  const periodos = useMemo(() => {
    if (modoTemporal === "rolling") {
      ...
      // Si el mes seleccionado es el mes en curso, acotar al día actual
      const esEsteMes = today.getFullYear() === y && today.getMonth() === m - 1;
      if (esEsteMes) {
        actualEnd = new Date(today);
        actualEnd.setHours(23, 59, 59, 999);
        diasActual = today.getDate();
        // Periodo previo: mismo rango 1..diaActual del mes anterior (comparación justa)
      ...
      // Mes pasado → comparar mes completo vs mes anterior completo
      const prevStart = new Date(y, m - 2, 1, 0, 0, 0, 0);
  ...
  // ─── Cálculo de stats para un rango ───
  const calcStatsRango = useCallback((start, end, diasReales) => {
    ...
    for (const row of data) {
      ...
      if (!porSucursal.has(suc)) {
        porSucursal.set(suc, {
          suc,
          kmScaniaCom: 0, kmScaniaVac: 0, kmVolvoCom: 0, kmVolvoVac: 0,
          tramosCom: 0, tramosVac: 0, tractos: new Set(),
        });
      }
    ...
    // Por sucursal — usa rend por marca (preciso)
    const sucursalArr = [];
  ...
  // ─── Tendencia semanal: últimas 10 semanas móviles ───
  const tendenciaSemanal = useMemo(() => {
    ...
    for (let i = 9; i >= 0; i--) {
      ...
      weeks.push({
        weekStart, weekEnd, kmCom, kmVac,
        kmTotal: kmTotalW, pctVacio: pctVac,
        costo: costoW, litros: litrosW, tramos,
      });
    }
    return weeks;
  }, [data, today, rendScania, rendVolvo, precioLitro, statsActual]);

  // ─── Insight de capacidad ───
  ...
  // Delta badge — "inverso" indica que un delta negativo es BUENO (ej: % vacío, costo/km)
  ...
  const pctVacioGlobal = statsActual.pctVacio;
  const costoVacioActual = (statsActual.kmScaniaVac / rendScania + statsActual.kmVolvoVac / rendVolvo) * precioLitro;

  // KPIs con delta
  const kpis = [
    {
      label: "KM/tracto/día", icon:"🎯",
      valorActual: statsActual.kmPorTractoDia.toFixed(0),
      unidad: "km",
      deltaVal: getDelta(statsActual.kmPorTractoDia, statsPrev?.kmPorTractoDia),
      color: T.blu,
      destacado: true,
      tooltip: "Indicador clave de capacidad: Σ km ÷ (tractos activos × días)",
    },
    {
      label: "KM Totales", icon:"🛣️",
      valorActual: fmtKmK(statsActual.kmTotal),
      deltaVal: getDelta(statsActual.kmTotal, statsPrev?.kmTotal),
      color: T.tx,
    },
    {
      label: "Costo Total", icon:"💰",
      valorActual: fmtCLP(statsActual.costoTotal),
      deltaVal: getDelta(statsActual.costoTotal, statsPrev?.costoTotal),
      color: T.red,
      inverso: true,
    },
    {
      label: "Litros Totales", icon:"⛽",
      valorActual: Math.round(statsActual.litrosTotal).toLocaleString("es-CL")+" L",
      deltaVal: getDelta(statsActual.litrosTotal, statsPrev?.litrosTotal),
      color: T.ac,
      inverso: true,
    },
    {
      label: "% Vacío", icon:"🔄",
      valorActual: statsActual.pctVacio.toFixed(1)+"%",
      deltaVal: statsPrev ? (statsActual.pctVacio - statsPrev.pctVacio) : null,
      deltaPP: true,
      color: statsActual.pctVacio > 25 ? T.red : statsActual.pctVacio > 15 ? T.ac : T.grn,
      inverso: true,
    },
    {
      label: "Costo/KM com.", icon:"💵",
      valorActual: fmtCLP(statsActual.costoPorKmComercial),
      deltaVal: getDelta(statsActual.costoPorKmComercial, statsPrev?.costoPorKmComercial),
      color: T.tx,
      inverso: true,
    },
    {
      label: "Rend. Prom.", icon:"📊",
      valorActual: statsActual.rendPromedio.toFixed(2)+" km/L",
      deltaVal: getDelta(statsActual.rendPromedio, statsPrev?.rendPromedio),
      color: T.blu,
    },
  ];
  ...
  return (<div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"12px",flexWrap:"wrap"}}>
        <div style={{display:"flex",background:T.sf2,borderRadius:"8px",padding:"3px",border:`1px solid ${T.bd}`}}>
          {[{k:"rolling",l:"Ventana móvil",ic:"🔄"},{k:"calendar",l:"Mes calendario",ic:"📅"}].map(o => (
            <button key={o.k} onClick={() => setModoTemporal(o.k)} style={{
              padding:"6px 14px",borderRadius:"6px",border:"none",cursor:"pointer",
              fontSize:"11px",fontWeight:600,fontFamily:"inherit",
              background: modoTemporal===o.k ? T.ac : "transparent",
              color: modoTemporal===o.k ? "#000" : T.txM,
              transition:"all 0.15s",
            }}>{o.ic} {o.l}</button>
          ))}
        </div>
        ...
        <div style={{marginLeft:"auto",fontSize:"11px",color:T.txM,display:"flex",gap:"12px",flexWrap:"wrap"}}>
          <span>
            <span style={{display:"inline-block",width:"8px",height:"8px",borderRadius:"2px",background:T.blu,marginRight:"4px"}}/>
            <strong style={{color:T.blu}}>Actual:</strong> {periodos?.labelActual}
          </span>
          <span>
            <span style={{display:"inline-block",width:"8px",height:"8px",borderRadius:"2px",background:T.txM,marginRight:"4px"}}/>
            <strong style={{color:T.txM}}>Previo:</strong> {periodos?.labelPrev}
          </span>
        </div>
      </div>
    </div>
    ...
    <div style={{...card,padding:"0",marginBottom:"12px",overflow:"hidden"}}>
      <button onClick={() => setParamsAbiertos(p => !p)} style={{
        ...
        <span style={{fontSize:"12px",fontWeight:600,color:T.tx,display:"flex",alignItems:"center",gap:"8px"}}>
          ...
          <span style={{fontSize:"11px",color:T.txM,fontWeight:400}}>
            Scania {rendScania} · Volvo {rendVolvo} · ${precioLitro.toLocaleString("es-CL")}/L
          </span>
        </span>
      ...
      {paramsAbiertos && (
        <div style={{padding:"0 16px 16px",borderTop:`1px solid ${T.bd}`,marginTop:"4px",paddingTop:"14px"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"12px"}}>
            <div>
              <label style={{fontSize:"10px",color:T.txM,textTransform:"uppercase",letterSpacing:"0.5px"}}>REND. SCANIA (km/L)</label>
              <input type="number" step="0.1" min="0" value={rendScania}
                onChange={e => setRendScania(+e.target.value || 0)} style={input}/>
            </div>
            <div>
              <label style={{fontSize:"10px",color:T.txM,textTransform:"uppercase",letterSpacing:"0.5px"}}>REND. VOLVO (km/L)</label>
              <input type="number" step="0.1" min="0" value={rendVolvo}
                onChange={e => setRendVolvo(+e.target.value || 0)} style={input}/>
            </div>
            <div>
              <label style={{fontSize:"10px",color:T.txM,textTransform:"uppercase",letterSpacing:"0.5px"}}>PRECIO DIESEL ($/L)</label>
              <input type="number" step="1" min="0" value={precioLitro}
                onChange={e => setPrecioLitro(+e.target.value || 0)} style={input}/>
            </div>
          </div>
          <div style={{fontSize:"10px",color:T.txM,marginTop:"8px"}}>
            Los parámetros se guardan localmente en el navegador. Se recuerdan entre sesiones.
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"12px",marginBottom:"16px"}}>
      {kpis.map(k => (
        <div key={k.label} style={{
          ...card, marginBottom:0, padding:"16px",
          borderLeft: k.destacado ? `4px solid ${k.color}` : `1px solid ${T.bd}`,
          background: k.destacado ? T.sf : T.sf,
        }} title={k.tooltip || ""}>
          <div style={{fontSize:"10px",color:T.txM,marginBottom:"6px",textTransform:"uppercase",letterSpacing:"0.8px"}}>
            {k.icon} {k.label}
          </div>
    ...
    {insightCapacidad && (
      <div style={{
        ...card,
        borderLeft: `4px solid ${insightCapacidad.tipo==="presion" ? T.red : insightCapacidad.tipo==="holgura" ? T.ac : T.grn}`,
      }}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:"16px"}}>
          <div style={{flex:"1",minWidth:"300px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px"}}>
              <span style={{fontSize:"18px"}}>{
                insightCapacidad.tipo==="presion" ? "🔴" :
                insightCapacidad.tipo==="holgura" ? "🟡" : "🟢"
              }</span>
              ...
              <span style={{
                ...
                background: insightCapacidad.tipo==="presion" ? `${T.red}22` : insightCapacidad.tipo==="holgura" ? `${T.ac}22` : `${T.grn}22`,
                color: insightCapacidad.tipo==="presion" ? T.red : insightCapacidad.tipo==="holgura" ? T.ac : T.grn,
                border: `1px solid ${insightCapacidad.tipo==="presion" ? T.red : insightCapacidad.tipo==="holgura" ? T.ac : T.grn}44`,
              }}>
                {insightCapacidad.tipo==="presion" ? "PRESIÓN" : insightCapacidad.tipo==="holgura" ? "HOLGURA" : "EQUILIBRIO"}
              </span>
            </div>
            <div style={{fontSize:"12px",color:T.tx,lineHeight:1.6}}>
              {insightCapacidad.mensaje}
            </div>
          </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"16px"}}>
      <div style={{...card,borderLeft:`4px solid ${T.blu}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",fontSize:"12px"}}>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>KM COMERCIAL</div>
            <div style={{fontWeight:700,color:T.grn,fontSize:"15px"}}>{fmtKm(statsActual.kmScaniaCom)}</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>KM VACÍO</div>
            <div style={{fontWeight:700,color:T.ac,fontSize:"15px"}}>{fmtKm(statsActual.kmScaniaVac)}</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>LITROS</div>
            <div style={{fontWeight:700,color:T.tx,fontSize:"15px"}}>{Math.round(statsActual.litrosScania).toLocaleString("es-CL")} L</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>COSTO</div>
            <div style={{fontWeight:700,color:T.red,fontSize:"15px"}}>{fmtCLP(statsActual.costoScania)}</div></div>
        </div>
      ...
      <div style={{...card,borderLeft:`4px solid ${T.ac}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",fontSize:"12px"}}>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>KM COMERCIAL</div>
            <div style={{fontWeight:700,color:T.grn,fontSize:"15px"}}>{fmtKm(statsActual.kmVolvoCom)}</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>KM VACÍO</div>
            <div style={{fontWeight:700,color:T.ac,fontSize:"15px"}}>{fmtKm(statsActual.kmVolvoVac)}</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>LITROS</div>
            <div style={{fontWeight:700,color:T.tx,fontSize:"15px"}}>{Math.round(statsActual.litrosVolvo).toLocaleString("es-CL")} L</div></div>
          <div><div style={{color:T.txM,fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.5px"}}>COSTO</div>
            <div style={{fontWeight:700,color:T.red,fontSize:"15px"}}>{fmtCLP(statsActual.costoVolvo)}</div></div>
        </div>
        <div style={{marginTop:"10px",paddingTop:"10px",borderTop:`1px solid ${T.bd}`,fontSize:"11px",color:T.txM}}>
    ...
    <div style={card}>
      ...
      <div style={{fontSize:"11px",color:T.txM,marginBottom:"14px"}}>
        Barras apiladas = km comercial (verde) + km vacío (naranja) · Línea punteada = % vacío semanal · Usa rendimiento ponderado del período actual para estimar costo
      </div>
    ...
    <div style={card}>
      ...
      <div style={{fontSize:"11px",color:T.txM,marginBottom:"12px"}}>
        Agrupado por sucursal de destino del viaje · Litros calculados por marca del tracto · Click columna para ordenar
      </div>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
          <thead><tr>
    ...
    {/* RANKING TRACTOS COLAPSABLE */}
    <div style={{...card,padding:"0",overflow:"hidden"}}>
      <button onClick={() => setRankingAbierto(a => !a)} style={{
        ...
        <span style={{fontSize:"14px",fontWeight:600,color:T.tx,display:"flex",alignItems:"center",gap:"8px"}}>
          ...
          <span style={{fontSize:"11px",color:T.txM,fontWeight:400}}>
            ({statsActual.ranking.length} tractos con viajes)
          </span>
        </span>
      ...
      {rankingAbierto && (
        <div style={{padding:"0 16px 16px",borderTop:`1px solid ${T.bd}`}}>
          <div style={{fontSize:"11px",color:T.txM,marginTop:"8px",marginBottom:"8px"}}>
            Top 20 · Click columna para ordenar
          </div>
                ...
                <tr key={r.pat} style={{background:i%2?(T.isDark?"#1a1e28":"#f8fafc"):"transparent"}}>
                  ...
                  <td style={td}>
                    <span style={{display:"inline-block",padding:"2px 8px",borderRadius:"4px",fontSize:"10px",fontWeight:600,
                      background:r.marca==="SCANIA"?`${T.blu}22`:`${T.ac}22`,
                      color:r.marca==="SCANIA"?T.blu:T.ac,
                      border:`1px solid ${r.marca==="SCANIA"?T.blu:T.ac}44`}}>
                      {r.marca}
                    </span>
                  </td>
                  <td style={{...td,textAlign:"right",color:T.grn}}>{fmtKm(r.kmCom)}</td>
...
// ─── Gráfico de tendencia semanal (componente auxiliar) ───
function TendenciaSemanalChart({weeks, T, fmtCLP}) {
  ...
  // Coordenadas del line chart % vacío (para path SVG)
    ...
    <div style={{overflowX:"auto",paddingBottom:"8px"}}>
      <div style={{position:"relative",minWidth:totalW+"px",height:(H+60)+"px"}}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(p => (
          <div key={p} style={{
            position:"absolute",left:0,right:0,top:(H*(1-p))+"px",
            height:"1px",background:T.bd,opacity:0.4,
          }}/>
        ))}
        ...
        <div style={{display:"flex",alignItems:"flex-end",height:H+"px",gap:gap+"px",position:"relative",zIndex:1}}>
          {weeks.map(w => {
            ...
            return (
              <div key={w.weekStart.getTime()} style={{
                width:barW+"px",display:"flex",flexDirection:"column",justifyContent:"flex-end",
                position:"relative",
              }} title={
                `${fechaIni}—${fechaFin}\n`+
